...
export function resolveModelRuntime(model?: Model | null): ModelRuntimeCompatibility {
  ...
  if (model.isOpenSource || model.tags?.some((t) => t.toUpperCase().includes('OPEN WEIGHT') || t.toUpperCase().includes('OPEN-SOURCE'))) {
    ...
    return {
      ...
      defaultVramRequirement: model.hardwareRequirements?.vram || '8 GB',
      defaultRamRequirement: model.hardwareRequirements?.ram || '16 GB'
    };
  }
  ...
  return {
    ...
    recommendedTag: '',
    availableTags: [],
    reason: `Runtime not supported by this launcher yet. (${model.provider} is a hosted cloud API service, not an open-weights model)`
  };
}
